...
public class Connection
{
    ...
    /// </summary>
    public long LastActive { get; set; } = TimeHelper.GetNow();
    ...
    public void ShiftBuffer(int consumed)
    {
        ...
        var remaining = BytesRead - consumed;
    ...
    /// - 4KB → 8KB → 16KB → 32KB → 64KB → ... → MaxBufferSize
    ///
    /// Performance Characteristics:
    /// - Time: O(N) where N = current buffer size (copy existing data)
    /// - Space: Doubles memory usage (but capped at MaxBufferSize)
    ...
    {
        var currentSize = ReadBuffer.Length;
    ...
    /// </summary>
    private readonly ArrayBufferWriter<byte> _writeBuffer = new(1024);
    ...
    /// </summary>
    public int WriteBufferOffset { get; private set; }
    ...
    /// - Not Close() here - NetworkServer handles cleanup
    ...
    public bool Flush()
    {
        var writtenCount = _writeBuffer.WrittenCount;
